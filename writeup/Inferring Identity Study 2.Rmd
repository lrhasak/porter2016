...
library(dplyr)
####Import data
MTurk_Porter <- read.csv("/Users/lindseyhasak/Desktop/MTurk_PorterPilotA.csv", sep = ",") #This imports data from MTurk to check WorkerIDs against Qualtrix
View(MTurk_Porter)



mock.porter <- read.csv("/Users/lindseyhasak/Desktop/PorterPilotA.csv")#Import downloaded data from Qualtrix


#### Data exclusion / filtering
filtered.porter <- mock.porter %>% #Get rid of columns that won't matter for any data analysis
  select(-Start.Date,
         -End.Date,
         -Response.Type,
         -IP.Address,
...
colnames(filtered.porter)
         

#### Prepare data for analysis - create columns etc.
#Rename columns with long names to be shorter
new.porter <- rename(filtered.porter, Affirmative.Action = How.do.you.think.the.COMMUNICATOR.would.feel.about.eliminating.race.based.Affirmative.Action.,
...
#Select columns for main replication analyses
porter.communicator <- new.porter %>%
  select(Communicator.Party, LIB.Condition)
...
```{r}
#t-test
Communicator.Rating = t.test(Communicator.Party ~ LIB.Condition, data = porter.communicator)
