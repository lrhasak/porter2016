...
prereg <- read.csv("/Users/lindseyhasak/Desktop/PorterPreRegCleanTest.csv", header = TRUE) #Import downloaded data from Qualtrix

#### Data exclusion / filtering
filtered.pr <- prereg %>% #Get rid of columns that won't matter for any data analysis
  select(-Start.Date,
         -End.Date,
         -Response.Type,
         -Progress,
...
clean.pr <- rename(filtered.pr, Affirmative.Action = "How.do.you.think.the.COMMUNICATOR.would.feel.about.eliminating.race.based.Affirmative.Action.",
...
analysis.pr <- as.data.frame(clean.pr) %>% 
...
analysis.pr$AA.Rating <- as.numeric(analysis.pr$AA.Rating)
analysis.pr$LI.Rating <- as.numeric(analysis.pr$LI.Rating)
analysis.pr$IT.Rating <- as.numeric(analysis.pr$IT.Rating)
analysis.pr$ME.Rating <- as.numeric(analysis.pr$ME.Rating)
analysis.pr$CP.Rating <- as.numeric(analysis.pr$CP.Rating)
...
analysis.pr$LIB.Condition <- recodeLIB[clean.pr$LIB.Condition]
...
analysis.pr$Policy.Composite <- apply(analysis.pr[, c("LI.Rating",
                                                      "IT.Rating", 
                                                      "ME.Rating")],1,mean)
```
...
Policy.Rating = t.test(Policy.Composite ~ LIB.Condition, data = analysis.pr)
...
```{r}
AA.Trend = t.test(AA.Rating ~ LIB.Condition, data = analysis.pr)
